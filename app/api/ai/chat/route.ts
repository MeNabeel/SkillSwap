...
`.trim();
    ...
    // 4. System instructions for SkillSwap AI Assistant
...
Below is the authenticated user's REAL SkillSwap Profile Context:
${userContextText}
`.trim();

    // 5. Initialize server-side OpenAI client
      ...
      return NextResponse.json(
        {
          success: false,
          message:
            "OpenAI API key is not configured on the server. Please add OPENAI_API_KEY to your .env.local file.",
        },
        { status: 500 }
      );
    }
    ...
    try {
      ...
      // Fallback model attempt if primary model is unavailable
      console.warn("Primary OpenAI model failed, trying gpt-3.5-turbo fallback", modelError?.message);
      completion = await openai.chat.completions.create({
        model: "gpt-3.5-turbo",
        messages: [
          { role: "system", content: systemInstruction },
    ...
    console.error("AI Chat API Route error:", err?.message || err);
    return NextResponse.json(
      {
        success: false,
